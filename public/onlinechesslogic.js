...
function completePendingPromotionMove() {
    ...
    if (game.legalMoves().includes(pendingPromotionMove.from + pendingPromotionMove.to)) {
        // update to the new position
        board.set({
            fen: game.fen()
        });

        let moveSan = game.legalMovesSan().split(" ")[game.legalMoves().split(" ").indexOf(pendingPromotionMove.from + pendingPromotionMove.to)];

        if (!moveSan) {
            moveSan = game.legalMovesSan().split(" ")[game.legalMoves().split(" ").indexOf(pendingPromotionMove.from + pendingPromotionMove.to + "q")];
        }
...
var chessgroundconfig = {
    ...
    events: {
        move: (orig, dest) => {
            if (game.legalMoves().includes(orig + dest)) {
                let moveSan = game.legalMovesSan().split(" ")[game.legalMoves().split(" ").indexOf(orig + dest)];

                if (!moveSan) {
                    moveSan = game.legalMovesSan().split(" ")[game.legalMoves().split(" ").indexOf(orig + dest + "q")];
                }
...
function waitFfishLoad() {
    if (ffish != null) {
        ...
        onFfishInit();
        game = new ffish.Board("chess");
        chessgroundconfig.movable.dests = getDests();
...
function getDests() {
    ...
    let legalMoves = game.legalMoves().split(" ");
...
function moveMade(move) {
    ...
    if (game.legalMovesSan().includes(move)) {
        let moveLan = game.legalMoves().split(" ")[game.legalMovesSan().split(" ").indexOf(move)];
        game.pushSan(move);
        if (yourturn == 'b') {
            board.set({
                orientation: "black"
        ...
        board.move(moveLan[0] + moveLan[1], moveLan[2] + moveLan[3]);
        //board.set({lastMove: [moveLan[0] + moveLan[1], moveLan[2] + moveLan[3]]});
        board.set({
            turnColor: (game.turn() == true ? 'white' : 'black'),
            movable: {
                free: false, // all moves are valid - no validation
        ...
        /*if (move.includes('x')) {
            let element = document.getElementById('piececaptured' + (game.turn() == true ? "1" : "0"));
    ...
    board.set({check: (game.isCheck() ? (game.turn() == true ? 'white' : 'black') : false)});

    if (game.turn() == (yourturn == "w")) {
        if (selectedpremove) {
            let moveLan2 = game.legalMoves().split(" ")[game.legalMoves().split(" ").indexOf(selectedpremove.from + selectedpremove.to)];
            let moveSan2 = game.legalMovesSan().split(" ")[game.legalMoves().split(" ").indexOf(selectedpremove.from + selectedpremove.to)];

            if (!moveSan2) {
                moveSan2 = game.legalMovesSan().split(" ")[game.legalMoves().split(" ").indexOf(electedpremove.from + selectedpremove.to + "q")];
            }
            const moveinfo2 = {
                from: moveLan2[0] + moveLan2[1],
                to: moveLan2[2] + moveLan2[3],
                promotion: 'q',
                san: moveSan2
            };
            if (moveinfo2) {
...
function gameOver() {
    ...
    if (game.isGameOver()) {
        winner = game.result().split('-')[0] === "1" ? 'Vitória das Brancas' : (game.result().split('-')[0] === "0" ? 'Vitória das pretas' : 'Empate'); 
    }
...
setInterval(() => {
    try {
        document.getElementsByTagName('cg-container')[0].className = "defaultboard board" + getDimensions().width.toString() + "x" + getDimensions().height.toString();
    }catch{}
}, 500);
function StartGame(turn) {
    game = new ffish.Board(variant);
    gamestarted = true;
    yourturn = turn;
    if (yourturn == 'b') {
        board.set({
